...
#include <time.h>     //i added this
#include <unistd.h>   //i added this
#include <stdlib.h>
...
{
  char *rval = malloc(size + 1);

  if (0 == rval) {   //malloc was myMalloc, i added + 1       
...
int rolo_only_to_read () //could return Bool
{
  return(sflag == 1 || number_of_nonopt_args > 0);   //option_present(SUMMARYFLAG) || n_non_option_args() > 0
}
...

{
    ...
    //Bool not_own_rolodex;   //i commented this out        
    //char *user;
    FILE *tempfp;
    ...
    //mine
    int opt;
    while((opt = getopt(argc, argv, ":u:ls")) != -1){
      switch(opt){
        case 'l':
          lflag = 1;
          //printf("loption: %c\n", opt);
        ...
        case 's': 
          ...
          break;
        case 'u':
          //must be immediatly followed by a user name 
          uflag = 1;
          //printf("u arg: %s\n", optarg);
          strcpy(user, optarg);
          //printf("user: %s\n", user);
        ...
        case '?':
          ...
          fprintf(stderr,"Illegal syntax using -u option\nusage: %s\n",USAGE);
          roloexit(-1); //is this ok
          break;
      }
    }

    // for(; optind < argc; optind++){
    //   number_of_nonopt_args++;
    //   //look_for_person("Vin");
    ...
    if (!(read_only = rolo_only_to_read())) {
       ...
       if (lflag == 0) {  //!option_present(NOLOCKFLAG)
    ...
    // if (number_of_nonopt_args > 0) {   //n_non_option_args() > 0
    //    //print_people();
    //    //look_for_person("vin");  //pass in all the names from command line
    //    exit(0);
    // }
