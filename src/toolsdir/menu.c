...
int menu_match (int *ptr_rval,char **ptr_ur,char *prompt,int casetest,int isub,int r_no_match,int r_ambiguous,int n_options, ...)
     //va_dcl, 
     //va_alist //i commented these out
   //)
  ...
  // int r_ambiguous;
  // int n_options;
  //int va_dcl;     //i made this an int
  //va_alist;   //i added this

{
  char sline[MAX_MENU_RESPONSE_LENGTH];
  //char *option;
  char *options[MAX_MENU_OPTIONS] = {NULL};
  int rvals[MAX_MENU_OPTIONS];
  int j,found,len,optionlen,rval,compare,blankindex;
  //int *addr;
  va_list pvar; 
        
  if (n_options / 2 > MAX_MENU_OPTIONS) return(MENU_ERROR);
  for (j = 0; j < MAX_MENU_RESPONSE_LENGTH; j++) line[j] = ' ';
  
  /* grab all the menu options and return values.  */

  blankindex = -1;
  va_start(pvar, n_options);   //i added a second argument
  //printf("n_options: %d\n", n_options);
  //printf("j: %d\n", j);
  for (j = 0; j < n_options / 2; j++) {
      options[j] = va_arg(pvar, char *);
      //printf("j: %d\n", j); 
      //printf("option: %s\n", options[j]);
      //printf("string length: %ld\n", strlen(options[j]));
      if (strlen(options[j]) == 0) {
  ...
  printf("%s",prompt);
  switch (rval = getlineMine(stdin,line,MAX_MENU_RESPONSE_LENGTH)) {
    case AT_EOF :
      return(MENU_EOF);
      break;
    case TOO_MANY_CHARS :
...

{
  ...
  return(MENU_RETURN);  //will probably have to change this 
}
