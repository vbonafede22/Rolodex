...
//{
  ...
  while (T) {
    ...
    switch (len = getlineMine(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int getlineMine (FILE *iport,char *buffer,int buflen){
  // FILE *iport; 
  // char *buffer; 
  // int buflen;
...
int getlines (FILE *fp,int n,char ***ptr_lines,char *linebuf,int maxlinelen){
  ...
  int len;
  char *line;
  if (0 > (len = getlineMine(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...
{  
  ...
  while (T) {
    ...
    if (0 > (len = getlineMine(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
