...

{
  ...
  add_entry :   
  if(basicdata[N_BASIC_FIELDS -1] != NULL){
    free(basicdata[N_BASIC_FIELDS -1]);
  }
  ...
  if (add_the_entry()) {
    ...
    for(int i = 0; i <= j; i++){
      free(basicdata[i]);   //i added this 
    }
    //free(otherdata[j]);   //i added this 
    free(rlink -> entry);
    free(rlink);          //i added this 
    return;
  }
