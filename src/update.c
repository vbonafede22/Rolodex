...
char *get_new_value () 
{
  char buffer[200];
  int rval;
  switch (rval = getlineMine(stdin,buffer,199)) {
    case AT_EOF :
      user_eof();
      break;
    case TOO_MANY_CHARS :
...
Ptr_Rolo_Entry copy_entry (entry) Ptr_Rolo_Entry entry;
{
  ...
  for(int i = 0; i < n; i++){
    free(new_entry -> other_fields[i]);
  }
  free(new_entry -> other_fields);
    ...
    case MENU_MATCH :
      ...
      switch (menuval) {
        ...
        case U_ABORT :
          ...
          if (entry){
            if(entry -> other_fields != NULL){
              for(int i = 0; i < N_BASIC_FIELDS - 1; i++){
                if (entry -> basicfields[i])
                  free(entry -> basicfields[i]);  //this is causing a problem 
              }
            }
            free(entry);
          }
