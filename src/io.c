...

{
  ...
  //Ptr_Rolo_Entry oldentry;    //i commented this out
  //Ptr_Rolo_entry currententry;    // i commented this out 
  Ptr_Rolo_List newlink,rptr;
  char *next_field,*next_other;
  //char *oldname;    //i commented this out
  //char *currentname;    //i commented this out
  char **other_pointers;
  int n_entries = 0;
  ...
  //free(rolofiledata);   //i added this 
  //free(other_pointers); //i added this 
  //free(newlink);
  return(n_entries);
...
{
  //int semi = 0;      //i commented this out  
  int i;
  if(!value){
    value = "";
  }
  ...
  while (*value != '\0') {
    if (*value == ';') {
       ...
       //semi = 1;    //i commented this out
    }
    else {
       //semi = 0;    //i commmented this out
       putchar(*value++);
    }
  }
  putchar('\n');
...
void cathelpfile (filepath,helptopic,clear)   //i changed this to void, was int
